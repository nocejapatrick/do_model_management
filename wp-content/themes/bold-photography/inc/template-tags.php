...
if ( ! function_exists( 'bold_photography_entry_footer' ) ) :
	...
	function bold_photography_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'bold-photography' ) );
			if ( $tags_list  ) {
			...
			if ( ! is_wp_error( $categories_list ) ) {
				echo '<span class="cat-links">' . bold_photography_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', 'bold-photography-pro' ) . '</span>' . $categories_list . '</span>';
			}
		}
...
function bold_photography_footer_content() {
	...
	$footer_content = sprintf( _x( 'Copyright &copy; %1$s %2$s %3$s', '1: Year, 2: Site Title with home URL, 3: Privacy Policy Link', 'bold-photography' ), 
	esc_attr( date_i18n( __( 'Y', 'bold-photography' ) ) ),
	'<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) .
	 '</a>', function_exists( 'get_the_privacy_policy_link' ) ? get_the_privacy_policy_link() : '' );
